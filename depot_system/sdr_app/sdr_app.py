...
"""
Purpose: This app is used to connect the ground system to the depot via SDR or simulation
"""
...
class SDRApp(BaseApp):
    def __init__(self) -> None:
        ...
        super().__init__("depot.sdr_app")

    def setup(self):
        ip = self.config_params.sdr_tcp_client_ip
        port = self.config_params.sdr_tcp_client_port
        self.tcp_client = TCPClient(ip=ip, port=port, sender="depot.sdr_app")
        ...
        if len(self.command_queue):
            for msg in self.command_queue:
                if "ground." in msg.destination or "all" == msg.destination:
                    if "depot.sdr_app" not in msg.sender:
